...
static void* findChildMenuFunctional(void* self, void* tmenuItem,
    ...
    // Accept the best candidate if it has readable items; scoring only
    // breaks ties toward routing-like menus.
    if (best) return best;
    return NULL;
}
...
static bool isBounceSourceMenu(void* self) {
    ...
    // Every item was a routing label (and there were 2-3 of them).
